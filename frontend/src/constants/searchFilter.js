...
const createLowerOptions = () => {
  const result = RENT_FEE.map(ele => ({key: ele * 10000, value: CommonHelper.convertYen(ele * 10000)}));
  result.unshift({key: null, value: i18n.t('searchFilter.no_lower_limit')});
  return result;
};

export const UPPER_RENT_FEE_OPTIONS = createUpperOptions();

export const LOWER_RENT_FEE_OPTIONS = createLowerOptions();
...
const createUpperSizeOptions = () => {
  const result = SIZE_OPTIONS.map(ele => ({key: ele, value: `${ele}m2`}));
  result.unshift({key: null, value: i18n.t('searchFilter.without_limit')});
  return result;
};

const createLowerSizeOptions = () => {
  const result = SIZE_OPTIONS.map(ele => ({key: ele, value: `${ele}m2`}));
  result.unshift({key: null, value: i18n.t('searchFilter.no_lower_limit')});
  return result;
};
...
const createYearOptions = () => {
  const result = YEAR_OPTION_LIST.map(ele => ({
    key: moment().year() - ele,
    value: i18n.t('searchFilter.within_years', {years: ele}),
  }));
